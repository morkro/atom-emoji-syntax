...
class SettingsElement extends HTMLElement {
	createdCallback () {
		...
		this.$directory.addEventListener('directory-update', this.update)
		...
		this.languages.forEach(lang =>
			this.$languageSettings.appendChild(this.createPanel(lang))
		)
	}
	...
	createPanel (data = {}) {
		...
		const [$panelHeading, $panelBody] =
			Object.keys($wrapper.children).map($el => $wrapper.children[$el])
	...
	managePanelItem (event) {
		...
		if (isElement && target.nodeName === 'H2') {
			...
			const { userSettings } = getLanguage($parent.getAttribute('data-emoji-language'))
			...
			userSettings.set('active', !!target.classList.contains('selected'))
