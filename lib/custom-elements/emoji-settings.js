...
class SettingsElement extends HTMLElement {
	...
	createPanel (data = {}) {
		...
		const [heading, body] = wrapper.children;
		// Grab .panel-heading
		heading.addEventListener('click', this.managePanelItem);
		// Grabs .panel-body and appends keywords
		this.createKeywordElements(data, body);
	...
	managePanelItem (event) {
		...
		if (isElement && target.nodeName === 'H2') {
			...
			language = languages.get(parent.getAttribute('data-emoji-language'));
