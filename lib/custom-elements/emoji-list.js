...
class ListElement extends HTMLElement {
	...
	setEmojiStyles (attr) {
		...
		return this.setAttribute('emoji-style', this.iconSet);
	}
		...
		this.emojiList[category].forEach(item => {
			list.appendChild(createNode('li', {
				append: createNode('span', {
					...
					textContent: item[0],
					attributes: { 'data-emoji-hexadecimal': item[1] }
				})
			}));
		});
	...
	updateList () {
		const emoji = this.querySelectorAll('span[data-emoji-hexadecimal]');

		[...emoji].forEach(item => {
			const unicode = item.getAttribute('data-emoji-hexadecimal');
			item.style.backgroundImage = `url(${unicode}.svg)`;
		});
	}
