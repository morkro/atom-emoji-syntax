...
class EmojiKeywordView extends HTMLElement {
	...
	createInputConfig () {
		const wrapper = createNode('div', {
			className: ['block', 'option-character']
		});

		wrapper.innerHTML = HTML`
			<div>Character:</div><atom-text-editor mini class="inline-block"></atom-text-editor>
		`;

		this.optionsWrap.appendChild(wrapper);
	}
	...
	manageSettingsData (event) {
		...
		if (isElement) {
			config.lang = this.getAttribute('data-emoji-language');
			config.keyword = this.getAttribute('data-emoji-keyword');

			// User toggles keyword state
			if (target.classList.contains('emoji-toggle-active')) {
				config.property = target.getAttribute('data-emoji-option');
				config.newValue = !!target.classList.contains('btn-primary');

				target.classList.toggle('btn-primary');
				this.updateLanguage(config);
			}
			// User toggles emoji positioning
				...
				config.newValue = target.getAttribute('data-emoji-position');
