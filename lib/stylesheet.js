...
const styleSheetSelector = 'emoji-syntax-stylesheet'
...
export function getContent () {
	...
	allLanguages.forEach(lang =>
		Object.keys(lang)
			.filter(item => !!lang[item].emoji)
			.filter(item => lang[item].active)
			.forEach(item => {
				content += createSelector({
					language: lang.languageSelector,
...
function setStyleSheetElement () {
	styleSheetElement = createStyleSheet({ content: getContent() })
}
...
export function styleSheetExists () {
	return !!atom.stylesElement.querySelector(`.${styleSheetSelector}`)
}
...
export function removeStyleSheet () {
	if (styleSheetExists()) {
		atom.styles.removeStyleElement(styleSheetElement)
		return true
	}
	return false
}
...
export function addStyleSheet () {
	if (!styleSheetExists()) {
		...
		atom.styles.addStyleElement(styleSheetElement)
		return true
	}
	return false
}
