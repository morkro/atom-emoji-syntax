...
const EmojiStyleSheet = {
	...
	createSelector (config = {}) {
		...
		if (style !== 'native') {
			custom = `
				background-image: url(${this.getImagePath(style, emoji)});
	...
	/**
	 * Returns the image to custom emoji folder.
	 *
	 * @param {String} style - The folder name, e.g. "emoji-one/"
	 * @param {String} emoji
	 *
	 * @return {String}
	 */
	getImagePath (style, emoji) {
		const path = atom.packages.getPackageDirPaths();
		const unicode = getHexadecimalUnicode(emoji);
		return `${path}/emoji-syntax/styles/${style}/${unicode}.svg`;
	},
